...
case ${STEP} in
    ...
    3)
        ...
        python generate_lp_files_semantic.py \
          ...
          --output ${OUTPUT_DIR}
        echo "Step 3 completed. LP files created in: ${OUTPUT_DIR}/dpa_${TARGET_DPA//' '/'_'}"
        ;;
    4)
        echo -e "\n[Step 4] Running Deolingo solver..."
        # Check if LP files exist
        if [ ! -d "${OUTPUT_DIR}/dpa_${TARGET_DPA//' '/'_'}" ]; then
            echo "Error: LP files directory not found. Run Step 3 first."
            exit 1
        fi
        ...
        find ${OUTPUT_DIR}/dpa_${TARGET_DPA//' '/'_'} -name "*.lp" | while read lp_file; do
            req_id=$(basename ${lp_file} .lp | sed 's/req_//')
            
            echo "Processing DPA ${TARGET_DPA//' '/'_'}, Requirement ${req_id}..." | tee -a ${RESULTS_FILE}
