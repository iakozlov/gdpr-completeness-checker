...
case ${STEP} in
    ...
    5)
        ...
        TEMP_PARA_OUTPUTS=""
        for TARGET_DPA in "${TARGET_DPAS[@]}"; do
            TEMP_PARA_OUTPUT="${OUTPUT_DIR}/paragraph_metrics_${TARGET_DPA//' '/_}.json"
            if [ -z "$TEMP_PARA_OUTPUTS" ]; then
                TEMP_PARA_OUTPUTS="${TEMP_PARA_OUTPUT}"
            else
                TEMP_PARA_OUTPUTS="${TEMP_PARA_OUTPUTS},${TEMP_PARA_OUTPUT}"
            fi
            ...
            python paragraph_metrics.py \
              ...
              --output ${TEMP_PARA_OUTPUT} \
              --target_dpa "${TARGET_DPA}"
        done
        
        # Aggregate paragraph results
        python aggregate_paragraph_metrics.py \
          --input_files "${TEMP_PARA_OUTPUTS}" \
          --output ${PARAGRAPH_OUTPUT}
        
        echo "Step 5 completed. Aggregated paragraph metrics saved to: ${PARAGRAPH_OUTPUT}"
        ;;
    [aA])
